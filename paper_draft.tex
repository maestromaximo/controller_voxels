...
\label{eq:ss_ui_star}
\end{align}
\todo[inline]{fixed environment term up to here}
...
LQR controllers are ubiquitous, though a more general approach is required to see improvements in aspects of stability\todo[inline]{i mean an avoidance of transit between oscillatory states and non-oscillatory states (R problem). There's probably a better way to state this}, settle speed, tuning ambiguities, obedience of constraints, and self-correction in the presence of non-linear behaviour. Given the original problem statement, the solution to the LQR required the constraints to be reformulated into a softer form, with immediate and direct consequences on its qualitative performance. The model predictive control (MPC) technique is better-suited to this problem, handling constraints directly and recalculating the (potentially nonlinear) control law at every timestep for optimisation over a finite horizon. \todo[inline]{check: Further, the resulting control law is generally piecewise linear for linear models.} The key idea of MPC is that it, with a prediction of the system's response to a constant variation on the inputs, optimises a cost with some finite time horizon. Reframing the original problem statement such that the cost function only depends on the final state (${J=(\vy(H, \vu^*) - \vys(H))^\text{T}\mathbf{Q}(\vy(H, \vu^*) - \vys(H)}$ for some finite horizon $H$), we can guarantee that an optimal controller reaches the desired state (or as close to it as allowed) within a finite time. Due to the repeating action, the quantity $\Tilde{J}=\sum_t\sum_i(\vy(H+t, \vu^*) - \vys(H))^\text{T}\mathbf{Q}(\vy(H+t, \vu^*) - \vys(H)$, effectively the discretised version of~\ref{eqn:costfunc}, is minimised.

For the generalised problem statement consisting of a cost function dependent on the state vector, inputs, rates of change of either, and constraints thereon, the MPC is typically not analytically solvable. Instead, iterative approaches are used; a rich portion of the existing literature is focussed on convergence, existence and uniqueness of solutions, and stability. 

In the case, however, of an optimal control problem with a cost of the squared errors between a target state and the state at some time $\tau$ in the future at every timestep, under constraints on controlled variables only, there does exist an analytical solution. We will use this to build the solution to our discretised problem:

\begin{align}
  \text{Find control sequence}\quad &\vu[t] = \arg\min_{\vec{u'}[t]\in\Omega} J(\vec{u'}[t]),\\[4pt]
  J(\vu[t]) &= \sum_{n=1}^{H} \Big( (\vy(n\Delta t)-\vys)^\text{T} \mathbf{Q}\, (\vy(n\Delta t)-\vys)\Big)\,,\\[6pt]
  \text{subject to}\quad &\dot{\vy}(t) = \mathbf{A}\,\vy(t) + \mathbf{B}\,\vut + \mathbf{E},\\[4pt]
  \text{and}\quad& \Omega \equiv [0, p_i]^N,
\end{align}
with $\Delta \vec{u}[\tau]=\vec{u}[\tau+1]-\vec{u}[\tau]$ defining discrete changes in $\vu(t)$ taking place at time $\tau$.

Being somewhat out of scope and primarily results of linear programming, we will use the results of Ref.~\cite{gupta1997analytical} without proof: For step response matrix $\mathbf{S}$, diagonal cost matrix $\mathbf{Q}$, the optimal control move at current time $t$ is
\begin{align}
    \Delta \vu[t]&=(\mathbf{Q} \mathbf{S})^{-1}(\vec{e} + \vec{e}_C)
\end{align}
constrained to $\Delta \vu \in [-\vu, \vec{p}-\vu]$ at every timestep, where
\begin{align}
    \vec{e}_C&=\mathbf{Q} (\vys - \vy_\text{traj}[t+H])
\end{align}
is the predicted error at the horizon, and $\vy_\text{traj}[t+H]$ is the predicted state arising at the time horizon, arising from only previous control moves: (see Appendix~\ref{appendix:trajectories}, eqn.~\ref{eqn:trajectoryprediction})
\begin{align}
    \vy_\text{traj}[t+H]=\mathbf{A}^{-1}\mathbf{\Phi}(H) 
...
\end{align}
This provides an optimal controller to calculate a control move $\Delta \vu$ at every timestep.\todo[inline]{Discussion on the choice of horizon - the system will always settle back to ``room temperature'', so to speak, so cost at horizon might be completely constant depending on choice... Build up a sum of these?}
\todo[inline]{We have several options. See below:}
\begin{itemize}
    %\item A sum of costs at every timestep (very true to our original cost functional)
    %\item A sum of costs at some average time constant for our system (has the drawback of performing badly for very slow responses, but very quick ones will settle at their newly perturbed temperature quickly (since we're dealing in $\Delta \vu$) and they'll be minimised well.
    %\item ...
    %\item Maybe this cost function actually works well, since we're working with $\Delta \vu$
    %\item Higher horizons will essentially ignore noise more, though, since any artificial perturbations will settle.
    \item Final conclusion: Set horizon to somewhere like our ``average'' time constant, as well as maximum. Compare numerically. Include a discussion on how the cost function should be somewhat ``close'' in some sense to the original one, or perhaps even more harsh.
    \item Additionally, it can't hurt to see how modifying $e_C$ to include earlier-than-horizon cost can change the control behaviour. Analysis is still to be done.
\end{itemize}
...
\begin{itemize}
    ...
    \item Also another one with $e_C=\mathbf{Q}\left(\vys - \sum_m^M \vy_\text{traj}[t + n\Delta t] f[n]\right)$ for several different $M$, and $f[m]=1/M$, $f[m]=(e^{(m-M)})/\sum_i^M e^{(i-M)}$ (i.e., just some \emph{normalised} weight functions)
\end{itemize}
...
\section{Derivation of Trajectory and Step Response}\label{appendix:trajectories}
...
\begin{align}
    \vy_\text{step}(H) &= \mathbf{A}^{-1} 
        ...
        +
        \mathbf{\Phi}(H)\vec{y}_0\label{eqn:stepresponse}
\end{align}
This result is, of course, equivalent to changing $\vu(t)$ by some $\Delta \vu$ at time $t=0$.
