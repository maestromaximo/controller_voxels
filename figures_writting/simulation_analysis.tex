...
Finally, we validated the numerical stability of the controller over a 1-hour horizon (3600 seconds) while maintaining a spatial stripe pattern (alternating columns of $80^\circ$C and $40^\circ$C). Figure~\ref{fig:sim8} demonstrates that the system settles into a robust steady state. The traces (Top) show no drift or oscillation. The RMS error (Bottom) converges to a much lower value ($\approx 8^\circ$C) compared to a full checkerboard pattern, as the striped configuration reduces the thermal load on the cold voxels by providing neutral neighbors along the vertical axis.
