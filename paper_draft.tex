...
\begin{align}
    \mathbf{S}&=\mathcal{B}\mathbf{A}^{-1}\left(\mathbf{\Phi}(H) - \mathds{1}\right)\mathbf{B}
\end{align}
with the preceding matrix (in block form) $\mathcal{B}=\begin{bmatrix}
        \mathds{1}\\0
    \end{bmatrix}$ added to specify that the step response matrix relates inputs ($\vu$) to temperature readings ($\vec{T}$).
