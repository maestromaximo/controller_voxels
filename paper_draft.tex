...
\delta\dot{T}_i &=
  ...
  -\kappa_i\,\delta T_i
  +\frac{\kappa_i}{c_i^*}\,\delta E_i
  -k_i^e \delta T_i,
  \label{eq:deltaT_dyn}\\[2pt]
\delta\dot{E}_i &=
...
&=
\begin{cases}
-\displaystyle\sum_{j'\neq i} k_{ij'} - \kappa_i - k_i^e, & i=j,\\[6pt]
k_{ij}, & i\neq j,
\end{cases}
...
In the case, however, of an optimal control problem with a cost of the squared errors between a target state and the state at some time $\tau$ in the future, under constraints on controlled variables only, there does exist an analytical solution. We will use this to build the solution to our discretised problem, leveraging the error-state formulation developed previously:

\begin{align}
  \text{Find control move sequence}\quad &\delta\vu(t) = \arg\min_{\delta\vec{u'}[t]\in\Omega'} J(\delta \vec{u'}[t]),\\[4pt]
  J(\delta\vu(t)) &= \sum_{n=1}^{\tau} \Big( (\delta\vy(n\Delta t))^\text{T} \mathbf{Q}\, (\delta\vy(n\Delta t))\Big)\,,\\[6pt]
  \text{subject to}\quad &\delta\dot{\vy}(t) = \mathbf{A}\,\delta\vy(t) + \mathbf{B}\,\delta\vut,\\[4pt]
  \text{and}\quad& \Omega' \equiv [-u_i^*, p_i-u_i^*]^N,
\end{align}
where $\delta \vy[n]=\vy(n\Delta t)-\vys$ is the sampled error state, $\delta \vu[n]=\vu(n\Delta t)-\vu^{\,*}$ is the deviation input, and the discrete control move is $\Delta \vu[n]=\delta\vu[n]-\delta\vu[n-1]$. The box constraints $\Omega'$ are precisely the bounds in~\eqref{eq:delta_u_constraints} expressed in deviation coordinates.

Being somewhat out of scope and primarily results of quadratic programming, we will use the results of Ref.~\cite{gupta1997analytical} without proof: For the step response matrix $\mathbf{S}$ (derived in Appendix~\ref{appendix:trajectories}), and a diagonal cost matrix $\mathbf{Q}$, the optimal control move at current time $t$ is
\begin{align}
    \Delta \vu[t]&=(\mathbf{Q} \mathbf{S})^{-1}\vec{e}_C,\label{eq:mpc_control_move}
\end{align}
with $\Delta \vu$ constrained elementwise as in~\eqref{eq:delta_u_constraints}.  The correction vector is
\begin{align}
    \vec{e}_C&=\mathbf{Q}\big(\delta\vy^{\,*}-\delta\vy_{\text{proj}}[t+H]\big)
              =-\mathbf{Q}\,\delta\vy_{\text{proj}}[t+H],
\end{align}
since the steady error $\delta\vy^{\,*}$ is the origin in deviation coordinates.  The projected error state at the horizon $H$ now carries the deviation notation explicitly,
\begin{align}
    \delta\vy_{\text{proj}}[t+H]
        &=\mathbf{A}^{-1} 
        \left(\mathbf{\Phi}(H) - \mathds{1}\right)\mathbf{B}\,\delta \vu(t)
        +
        \mathbf{\Phi}(H)\,\delta\vec{y}(t),
        \label{eq:delta_y_proj}
\end{align}
which matches the nominal expression from Appendix~\ref{appendix:trajectories} because the linearised deviation dynamics remove the constant forcing terms.  Consequently, the step-response matrix retaining the familiar form $\mathbf{S}=\mathbf{A}^{-1}\left(\mathbf{\Phi}(H)-\mathds{1}\right)\mathbf{B}$ makes intuitive sense: in deviation space, the future prediction depends only on the current error and the incremental move.  Equations~\eqref{eq:mpc_control_move}--\eqref{eq:delta_y_proj} therefore provide an implementable controller that computes $\Delta \vu$ at every timestep while respecting the box constraints.
...
\begin{itemize}
    ...
    \item Also another one with $e_C=\mathbf{Q}\left(\vys - \sum_m^M \delta\vy_{\text{proj}}[t + n\Delta t] f[n]\right)$ for several different $M$, and $f[m]=1/M$, $f[m]=(e^{(m/M-1)})/\sum_i^M e^{(i/M-1)}$ (i.e., just some \emph{normalized} weight functions)
\end{itemize}
...
The system evolves according to the linear, piecewise-constant coefficient, non-homogeneous equation~\ref{eqn:evolutionlaw}. The homogeneous solution for $\delta\dot\vec{y}=\mathbf{A} \delta\vec{y}$ is trivially $\delta\vec{y}_H(t)=\mathbf{\Phi}(t)\delta\vec{y}_0$ with $\mathbf{\Phi}(t)=e^{\mathbf{A} t}$, and we will assume a particular solution of form $\delta\vec{y}_P(t)=\mathbf{\Phi}(t) \vec{v}(t)$. Plugging this into the original evolution law, and breaking the integral involving $\delta\vec{u}(t)$ into domains where $\delta\vec{u}$ remains constant, yields a formula:
\begin{align*}
    \vec{v}(t)&=\mathbf{\Phi}(t)\int_0^t \mathbf{\Phi}^{-1}(t') (\mathbf{B} \delta\vec{u}(t)) dt\\
    &=(\cdots)\\
    &=\mathbf{A}^{-1} 
        \left( \sum_{n=1}^{t/\Delta t}\left(\Big( 
            \mathbf{\Phi}\left(t - (n-1)\Delta t\right) - 
            \mathbf{\Phi}\left(t - n \Delta t\right)
            \Big) \mathbf{B} \delta\vec{u}[n]\right) 
        \right)
\end{align*}
Which could possibly have been intuited, and can easily be interpreted as exponentially-decaying weightings on past inputs (with an additional perturbation from $\mathbf{E}$, should we have not worked with the error state). To satisfy the initial condition $\delta\vec{y}_0$, the full solution is then
\begin{align}
    \delta\vec{y}(t)&=
        \mathbf{\Phi}(t)\delta\vec{y}_0+
         \mathbf{A}^{-1} \sum_{n=1}^{t/\Delta t}\left(\Delta \mathbf{\Phi}[n] \mathbf{B} \delta\vec{u}[n]\right)
        \label{eqn:trajectoryprediction}
\end{align}
defining $\Delta\mathbf{\Phi}[n]=\Big( 
            \mathbf{\Phi}\left(t - (n-1)\Delta t\right) - 
            \mathbf{\Phi}\left(t - n \Delta t\right)
            \Big)$. 
Is is notable that $\mathbf{A}^{-1}$ is known to exist in our case, since the underlying physical system is assumed to be valid, and thus the rank of the matrix is exactly equal to its dimension. Further, $\mathbf{A}^{-1}$ commutes with $\mathbf{\Phi}$ 

Further, we can now derive the step response of the system; for $\delta\vu$ from time $t=0$ to $t=H$, with previous input $\delta u=0$ (corresponding to $\vy_\text{opt}$), the state will evolve as:
\begin{align}
    \delta\vy_\text{step}(H) &= 
        \mathbf{\Phi}(H)\vec{y}_0+\mathbf{A}^{-1} 
        \left( \sum_{n=1}^{H/\Delta t}\left(\Delta \mathbf{\Phi}[n] \mathbf{B} \delta\vu\right) 
        \right)\nonumber
        \\
        &=\mathbf{\Phi}(H)\vec{y}_0+\mathbf{A}^{-1} 
        \left(\mathbf{\Phi}(H) - \mathds{1}\right)\left(\mathbf{B} \delta\vu \right)
        \\
    (\vy_\text{step}(H) - \vy_\text{opt}(H)) &= \mathbf{A}^{-1} 
        \left(\mathbf{\Phi}(H) - \mathds{1}\right)\mathbf{B} \delta\vu\label{eqn:stepresponse}
\end{align}
This result is, of course, equivalent to changing $\delta\vu(t)$ by some $\Delta \vu$ at time $t=0$. Hence, the step response matrix is
\begin{align}
